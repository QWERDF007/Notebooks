...
**表 12：** 当仅用 *train_fine* 集训练时 DeepLabv3 在 Cityscapes 验证集上的性能。**OS：** $output\_stride$。**MS：** 推理期间多尺度输入。**Flip：** 添加左右翻转的输入。
...
**表 13：** 在 Cityscapes 测试集上的性能。**Coarse：** 也使用 *train_extra* 集 (粗标注)。这个表中只列出了一些已知的顶级模型。
