...
受到使用一种不同大小网格的层级结构的多网格方法和随后的 [84, 18] 所激励，我们在所提出的模型中的 block4 到 block7 中采用不同空洞率。特别地，我们将 $Multi\_Grid = (r_1, r_2, r_3)$ 定义为 block4 到 block7 内的三个卷积层 (每个 block 各有 3 个 conv) 的 unit rates。卷积层的最终空洞率等于 unit rate 和相应的空洞率 rate 的乘积。例如，当 $output\_stride = 16$ 和 $Multi\_Grid = (1, 2, 4)$，block4 中三个卷积的空洞率分别是 $rate = 2 \cdot (1, 2, 4) = (2, 4,8)$。

### 3.3. Atrous Spatial Pyramid Pooling

我们重新审视了 [11] 中提出的空洞空间金字塔汇聚，其中四个并行的具有不同空洞率的空洞卷积被应用于特征映射上。ASPP 的灵感来自于空间金字塔汇聚的成功 [28,49,31]，该方法表明在不同尺度上重新采样特征对于准确地和高效地分类任意尺度的区域是有效的。不同于 [11]，我们在 ASPP 中包含了批量规范化 (BN)。
