...
第三个是网络中远距离的依赖关系之间的路径长度。在许多序列转录任务中，学习远距离的依赖是一个关键的挑战。影响学习这种依赖关系能力的一个关键的因素信号是在网络中不得不遍历的的前向和后向路径的长度。输入和输出序列中任意位置的组合之间的路径越短，就越容易学习远距离的依赖。因此我们还比较了由不同层类型组成的网络中的任意两个输入和输出位置之间的最大路径长度。

如表 1 所示，一个自注意力层用一个恒定数量的时序性 executed 操作将所有的位置连接起来，而循环层需要 $O(n)$ 的时序性操作。在计算复杂度方面，当序列的长度 $n$ 小于表示的维度 $d$ 时，自注意力层比循环层更快，这在机器转录的 state-of-the-art 模型中所使用的句子表示 (如 word-piece 和 byte-pair 表示) 中是最常见的情况。为了提升涉及非常长的序列任务的计算性能，自注意力可以被限制为只考虑输入序列中以各自的输出位置为中心的大小为 $r$ 的领域。这将提升最大的路径长度到 $O(n/r)$。我们计划在未来的工作进一步研究这种方法。

一个核宽度 $k < n$ 的单卷积层不会连接所有的输入和输出位置对。在连续的内核的情况下，要这样做需要堆叠 $O(n/k)$ 个卷积层，或者在使用空洞卷积的情况下需要 $O(log_k(n))$，这会增加网络中任意两位置之间的最长路径的长度。卷积层通常比循环层贵 $k$ 倍。然而，可分离的卷积讲复杂度大大降低到 $O(k \cdot n \cdot d + n \cdot d^2)$。然而，即使 $k = n$，可分离的卷积的复杂度也等于自注意力层和逐点的前馈层的组合，后者是我们模型中采用的方法。
...
**Residual Dropout**  我们将 dropout 应用于每个子层的输出，然后将它与子层的输入相加并规范化 (normalized)。此外，我们将 dropout 应用于堆叠的解码器和编码器中的嵌入和位置编码的和。对于基本模型，我们使用 $P_{drop} = 0.1$。
