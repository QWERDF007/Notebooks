...
**证明.** 对于 $p_g = p_{data}$， (\boldsymbol{x}) = \frac{1}{2}$ (考虑等式 (2))。因此，代入 $D^{\ast}_{G}(\boldsymbol{x}) = \frac{1}{2}$ 到等式 (4)，可以得出 $C(G) = \log \frac{1}{2} + \log \frac{1}{2} = -\log 4$ 。该值就是在 $p_g = p_{data}$ 时， $C(G)$ 所能取得的最优值，观察到
$$
