...
我们提出了一个简单易用的网络 (称为SimpleNet)，用于检测和定位异常。SimpleNet 由四个组件组成：(1) 一个预训练的特征提取器，用于生成局部特征，(2) 一个浅层特征适配器，用于将局部特征迁移到目标域，(3) 一个简单的异常特征生成器，通过在正常特征上添加高斯噪声来伪造异常特征，以及 (4) 一个二元的异常鉴别器，用于区分异常特征和正常特征。在推理期间，异常特征生成器将弃用。我们的方法基于三个直觉。首先，将预训练特征转换为目标导向特征，有助于避免域偏差。其次，在特征空间中生成合成异常比在图像空间中更有效，因为缺陷在图像空间中通常缺乏共性。第三，一个简单的鉴别器更加高效实用。尽管简单，但 SimpleNet 在定量和定性上均优于以前的方法。在 MVTec AD 基准测试中，SimpleNet 实现了99.6％ 的异常检测 AUROC，在减少错误 55.5％ 的同时比下一个最佳性能模型更好。此外，SimpleNet 比现有方法更快，在 3080ti GPU 上具有高帧率 77 FPS。此外，SimpleNet 在单类新颖性检测任务的性能方面也有显着改进。代码：https://github.com/DonaldRR/SimpleNet 。
...
图像异常检测和定位任务旨在识别异常图像并定位异常子区域。检测各种感兴趣的异常的技术在工业检查中具有广泛的应用[3, 6]。在工业场景中，由于异常样本稀少且异常可能从细微的变化（如细小划痕）到大型结构缺陷（例如缺失部分）不等，因此异常检测和定位尤其困难。图 1 展示了 MVTec AD 基准 [3] 中的部分示例以及我们方法的检测结果。这种数据分布使得完全监督学习难以实施。
...
**图 3**：提出的 SimpleNet 的概览。在训练阶段，将正常样本馈送到预训练的特征提取器中以获取局部特征。然后，使用特征适配器将预训练的特征适配到目标域。通过向适配后的特征添加高斯噪声来合成异常特征。适配后的特征和异常特征分别用作正样本和负样本，以训练最终的鉴别器。在推理期间，将删除异常特征生成器。
...
图 4 展示了异常特征对 MVTec AD 的四个类别影响。我们可以看到，适配后的特征的每个维度的标准差趋于一致。因此，在区分异常特征和正常特征时，特征空间趋于紧凑。

<img src="./assets/SimpleNet_fig4.png">

**图 4**：局部特征和适配后的特征的每个维度的标准差直方图。
...
本文所进行的实验配置和实现细节如下。所有实验中使用的骨干网络均在 ImageNet 上进行预训练的。当骨干网络是 ResNet-like 架构时，特征提取器中使用骨干网络的第 2 和第 3 中间层，即公式 3 中的 $l' \in [2, 3]$ 。默认情况下，我们的实现使用 WideResnet50 作为骨干网络，并将特征提取器中的特征维度设置为 1536。后面的特征适配器本质上是一个没有偏置的全连接层。特征适配器中 FC 层的输入和输出特征的维度相同。异常特征生成器向正常特征的每个条目添加 i.i.d. (独立同分布) 高斯噪声 $\mathcal N(0,\sigma^2)$ 。默认情况下， $\sigma$ 设置为 0.015。随后的鉴别器由一个线性层、一个批量归一化层 (BN)、一个 leaky relu (0.2 slope) 和一个线性层组成。公式 7中的 $th^+$ 和 $th^-$ 都设置为 0.5。使用 Adam 优化器，将特征适配器和鉴别器的学习率分别设置为 0.0001 和 0.0002，权重衰减设置为 0.00001。每个数据集的训练 epochs 设置为 160，批大小为 4。

### 4.4. Anomaly detection on MVTec AD

MVTec AD 上的异常检测结果如表 1 所示。图像级别的异常分数由异常图的最大分数给出，如公式 11 所示。SimpleNet 在15个类别中有 9 个类别获得了最高分数。对于纹理和物体，SimpleNet 分别达到了 99.8% 和 99.5% 的 I-AUROC 的新 SOTA。SimpleNet 实现了更高的平均图像异常检测性能，即 I-AUROC 得分为99.6%。请注意，从PatchCore [22] (在相同的 WideResnet50 骨干下，下一个最佳竞争对手) 的 0.9％ 误差降至 SimpleNet 的 0.4％，意味着错误率降低了55.5%。在工业检查设置中，这是一个相关且显著的减少。

<img src="./assets/SimpleNet_table1.png">

**表 1**：在 MVTec AD 数据集上的 SimpleNet 与最先进方法的比较。每个条目显示图像级别的 AUROC (I-AUROC) 和像素级别的 AUROC (P-AUROC)，格式为 I-AUROC%/P-AUROC%。文献[24]中没有记录 CS-Flow 的 P-AUROC。

<img src="./assets/SimpleNet_table2.png">

**表 2**：使用 WideResNet50 层级的不同组合在 MVTect AD 上的性能。

### 4.5. Anomaly localization on MVTec AD

异常定位性能是通过像素级 AUROC 来衡量的，我们将其称为 P-AUROC。与最先进的方法的比较如表 1 所示。SimpleNet 在 MVTec AD 上实现了 98.1% 的最佳异常检测性能，以及 98.4% 的物体 P-AUROC 的新 SOTA。SimpleNet 在 15 个类别中有 4 个类别获得了最高分数。我们在图 8 中展示了异常定位的代表性样本。

### 4.6. Inference time

除了检测和定位性能之外，推理时间是工业模型部署的最重要问题。图 2 显示了与最先进方法在推理时间上的比较。所有方法都在同一硬件上进行了测量，该硬件包含一个 Nvidia GeForce GTX 3080ti GPU 和一个 Intel(R) Xeon(R) CPU E5-2680 v3@2.5GHZ。结果清楚地表明，我们的方法同时实现了最佳性能和最快速度。SimpleNet 比 PatchCore [22]快近 8 倍。

### 4.7. Ablation study

**邻域大小和层次结构**。我们研究了公式 1 中邻域大小 $p$ 的影响。图 6 的结果显示了局部性和全局上下文之间在预测异常方面存在一个明确的最优点，这解释了为什么选择邻域大小 $p = 3$ 。我们设计了一组实验来测试层次子集 $L$ 对模型性能的影响，结果如表 2 所示。我们用 1-3 索引了前三个 WideResNet50 块。可以看出，第 3 层次的特征已经可以实现最先进的性能，但可以从第 2 层次中获益。我们选择 2 + 3 作为默认设置。

**适配器配置**。适配器提供了对预训练特征的转换 (投影)。我们的默认特征适配器是一个没有偏置的单个 FC 层，输入和输出通道相同。表 3 比较了不同特征适配器的效果，第一行 "Ours" 实现遵循与表 1 相同的配置。 "Ours-complex-FA" 用一个非线性适配器 (即具有非线性的单层 MLP) 替换了简单的特征适配器。"Ours-w/o-FA" 行删除了特征适配器。结果表明，单个 FC 层取得了最佳性能。直观地说，特征适配器找到了一个投影，使得伪造的异常特征和投影的预训练特征容易分离，这意味着对于鉴别器的有一个简单的解决方案。这也表明使用特征适配器有助于网络快速收敛 (图 7)。我们观察到使用复杂的特征适配器会导致显着的性能下降。一个可能的原因是复杂的适配器可能导致过拟合，减弱了对测试集各种缺陷的泛化能力。图 4 比较了特征适配器前后每个维度上标准差的直方图。我们可以看到，在使用异常特征进行训练时，适配后的特征空间变得紧凑。

<img src="./assets/SimpleNet_fig5.png">

**图 5**：使用不同 $\sigma$ 在 MVTec AD 数据集上每个类别的 I-AUROC% 和 P-AUROC%。

<img src="./assets/SimpleNet_fig6.png">

**图 6**：使用不同 patch 大小在 MVTec AD 上的性能。

<img src="./assets/SimpleNet_fig7.png">

**图 7**：训练过程中的损失可视化。绘制的线展示了 MVTec AD 数据集所有类别的平均损失。透明颜色显示了损失波动的范围。

**噪声尺度**。异常特征生成器中的噪声尺度控制合成的异常特征与正常特征的距离。具体而言，高 $\sigma$ 会导致异常特征与正常特征之间保持较大的欧式距离。在大 $\sigma$ 上进行训练将导致松散的决策边界，导致较高的假阴性率。相反，如果 $\sigma$ 很小，则训练过程将变得不稳定，并且鉴别器无法很好地推广到正常特征。图 5 详细说明了 MVTec AD 中每个类别 $\sigma$ 的影响。可以看出， $\sigma = 0.015$ 达到了平衡并产生了最佳性能。

<img src="./assets/SimpleNet_table3.png">

**表 3**：不同特征适配器的比较。"Ours" 实现遵循与表 1 相同的配置。"Ours-complex-FA"用非线性适配器替换了简单的特征适配器。"Ours-w/o-FA"删除了特征适配器，相当于使用恒等全连接层。"Ours-CE"使用交叉熵损失。 MVTec AD 的 I-AUROC％ 和 P-AUROC％ 如表所示。

<img src="./assets/SimpleNet_table4.png">

**表 4**：不同 backbone 在 MVTect AD 上的性能

<img src="./assets/SimpleNet_table5.png">

**表 5**：CIFAR-10 数据集的 one-class 新颖性检测 I-AUROC(%) 结果。

**损失函数**。我们将第 3.5 节中提出的损失函数与被广泛使用的交叉熵损失进行了比较 (表 3 中的 "Ours-CE" 所示)。我们发现，与交叉熵损失相比，改进了 0.2％ 的 I-AUROC 和 0.3％ 的 P-AUROC。

**对骨干网络的依赖性**。我们使用不同的骨干网络测试 SimpleNet，结果如表 4 所示。我们发现，选择不同的骨干网络结果大多稳定。选择 WideResNet50 是为了与 PaDiM [6]和 PatchCore [22] 进行比较。

**定性结果**。图 8 显示了异常定位结果，指示异常区域。分割结果的阈值是通过计算每个子类别的所有异常分数的 F1 分数获得的。实验结果证明，所提出的方法即使在相当困难的情况下也可以很好地定位异常区域。此外，我们可以发现，所提出的方法在物体和纹理类别中具有一致的性能。

<img src="./assets/SimpleNet_fig8.png">

**图 8**：定性结果，显示 MVTec AD 中每个类别的采样图像、真实标注和异常映射。

### 4.8. One-Class Novelty Detection

为了评估所提出的 SimpleNet 的普适性，我们在 CIFAR-10 [16]上进行了 one-class 新颖性检测实验。按照[19]的方法，我们使用来自单个类别的样本训练模型，并检测来自其他类别的新颖样本。我们分别为每个类别训练相应的模型。请注意，新颖性分数定义为相似性图中的最大分数。表 5 报告了我们的方法和其他方法的 IAUROC 得分。为了公平比较，所有方法都是在 ImageNet 上进行预训练的。基线包括 VAE [2]、LSA [1]、DSVDD [25]、OCGAN [19]、HRN [15]、AnoGAN [27]、DAAD [14]、MKD [26]、DisAug CLR [28]、IGD [5] 和 RevDist [7]。我们的方法优于这些比较方法。请注意，IGD [5] 和DisAug CLR [28]在自监督学习的帮助下分别达到了 91.25% 和 92.4%。
