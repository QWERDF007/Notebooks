...
我们使用不同的 backbone 测试了 PatchCore，结果显示在 S6 中。我们发现结果在不同 backbone 的选择上大多是稳定的。选择 WideResNet50 是为了与 SPADE 和 PaDiM 具有可比性。
...
查看图 S1，我们发现大多数假正例错误要么源自a) (用蓝色标出) 标签存在模糊、即也可能被标记为异常的图像变化，要么源自b) (用橙色标出) 非常高的标称方差，类似潜在异常。而前者几乎无法通过所提出的方法来处理，后者可以通过为标称数据提供某种形式的自适应来处理。然而，由于 PatchCore 的性能优于自适应方法，此类自适应与基于预训练的方法 (如 PatchCore) 的结合最有前景。
...
图 S1：剩余的假正例分类可视化 (基于 F1 最优阈值)。颜色表示不同的错误来源。橙色表示被误认为异常的高度标称方差，蓝色表示由于标注环境中的异常导致的错误分类，橄榄色表示被误认为异常内容的背景方差。
...
图 S2：剩余假反例分类的可视化(基于 F1 最优阈值)。颜色表示不同的错误来源。橙色表示被误认为异常的高度标称方差，绿色表示被定位但置信度不足的异常，粉色代表未被发现的异常，紫色代表因基于裁剪的图像处理而遗漏的异常 (总共一个)，灰色代表在更高分辨率下可被发现的细粒度异常。
