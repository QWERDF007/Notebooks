...
为了获得 $s$ ,我们使用缩放 $w$ 对 $s^∗$ 进行调整，来考虑邻域 patch 的行为：如果与异常候选 $m^{test,\star}$ 最接近的内存库特征 $m^{\star}$ 本身远离相邻样本，从而已经是一个罕见的正常样本，我们增加异常分数：
...
表 1 所示的是MVTec数据集上图像级异常检测的结果。对于PatchCore，我们报告了不同程度的内存库子采样 (25%、10%和1%)。在所有情况下，PatchCore都达到了显著更高的平均图像异常检测性能，在所有子数据集上表现始终很高 (请参见补充材料B中的详细比较)。请注意，错误率从 2.1% (PaDiM) 降低到PatchCore-25% 的 0.9%，意味着错误减少了57%。在工业检测设置中，这是相关和显著的减少。对于MVTec 在最佳 F1 阈值下，只有 42 张图像中的 1725 张被错误分类，三分之一的所有类别被完美解决。在补充材料 B 中，我们还显示了在 F1 最佳工作点和完全召回下，与 SPADE 和 PaDiM 相比，分类错误也更低。 使用 PatchCore，不到 50 张图像保持错误分类。此外，PatchCore 达到了最先进的异常分割性能，无论是通过像素级 AUROC (表 2，98.1 对 97.5 为PaDiM) 还是 PRO 指标 (表 3，93.5 对 92.1) 衡量。图1中的样本分割提供了对异常定位准确性的定性印象。
...
图5比较了不同的内存库 $\mathcal M$ 子采样方法：贪心核心集选择、随机子采样和学习一组对应于子采样目标百分比 $p_{target}$ 的基向量。对于后者，我们对 $p_i \in \mathcal P \subset \mathbb{R}^d$ 进行采样，其中 $\lvert \mathcal P \rvert = p_{target} \cdot \lvert \mathcal M \rvert $  ，然后任务是最小化一个基础重构目标
...
以找出最能描述内存库数据 $\mathcal M$ 的 $N$ 个基向量。在图 5 中，我们比较了这三种设置，发现基于核心集的子采样优于其他可能的选择。不进行子采样的性能与大小上较小两个数量级的核心集减少内存库相当。我们还发现子采样内存库包含的冗余较少。我们记录了在测试时使用的非子采样和核心集子采样内存库样本的百分比。尽管最初只使用不到 30% 的内存库样本，但核心集子采样 (到1%) 将这个因素提高到接近95%。对于某些子采样区间 (在大约 50% 和 10% 之间)，我们甚至发现与未采样的 PatchCore 相比，检测和定位的联合性能有所提高。最后，通过增加步幅 (参见公式3) 来减少内存库大小 $\mathcal M$ 表现较差,因为分辨率上下文减少，步幅 $s=2$ 给出图像异常检测 AUROC 为 97.6%，步幅 $s=3$ 给出 AUROC 为 96.8%。
