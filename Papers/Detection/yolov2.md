...
我们提出了一种联合训练分类和检测数据的机制。我们的方法利用标记用于检测的图像来学习特定于检测的信息，例如边界框坐标预测和目标性，以及如何对常见物体进行分类。它使用仅具有类别标签的图像来扩展其可以检测的类别数量。在训练过程中，我们将来自检测和分类数据集的图像混合在一起。当我们的网络看到一张标记用于检测的图像时，我们可以基于完整的 YOLOv2 损失函数进行反向传播。当它看到一张分类图像时，我们只会反向传播来自架构中特定于分类的部分的损失。
...
大多数分类方法都使用所有可能类别上的 softmax 层来计算最终的概率分布。使用 softmax 假设类别是互斥的。这给合并数据集带来了问题，例如，您不想使用此模型将 ImageNet 和 COCO 结合起来，因为“诺福克梗”和“狗”不是互斥的类别。相反，我们可以使用不假设互斥的多标签模型来组合数据集。这种方法忽略了我们所有已知的数据结构，例如 COCO 类别都是互斥的。

层次分类。ImageNet 标签来自 WordNet，这是一个用于构建概念及其关系的语言数据库 [12]。在 WordNet 中，“诺福克梗”和“约克夏梗”都是“梗”的下位词，“梗”是一种“猎犬”，猎犬是一种“狗”，狗是一种“犬科动物”等。大多数分类方法假设标签具有扁平结构，但是对于合并数据集，结构正是我们所需要的。WordNet 被组织成有向图，而不是树，因为语言很复杂。例如，“狗”既是“犬科动物”的一种，也是“家畜”的一种，它们在 WordNet 中都是同义词集。为了简化问题，我们没有使用完整的图结构，而是通过 ImageNet 中的概念构建了一个层次树。为了构建这个树，我们检查了 ImageNet 中的视觉名词，并查看它们在 WordNet 图中的路径到根节点（在本例中为“物理对象”）。许多同义词集只有一个路径穿过图表，因此我们首先将所有这些路径添加到我们的树中。然后，我们迭代地检查我们剩下的概念，并添加尽可能少地扩展树的路径。因此，如果一个概念有两个到根的路径，并且一条路径会向我们的树添加三个边，而另一条路径只会添加一个边，那么我们选择较短的路径。
