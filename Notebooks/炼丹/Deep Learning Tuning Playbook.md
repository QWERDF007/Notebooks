...
- 从长远来看，理解问题对于最终性能提升至关重要。优先考虑洞察力而不是短期收益可以帮助我们：
  ...
  - 避免仅仅因为历史偶然原因而在表现良好的运行中启动不必要的更改。
...
- 当我们准备好探索时，即使实验不能最大程度地提供有关调整问题结构的信息，我们也可以纯粹关注验证错误。

### 选择下一轮实验的目标

**总结**：每轮实验都应该有清晰的目标，并且范围足够狭窄，以至于实验能够真正朝着目标取得进展。

- 每轮实验都应该有清晰的目标，并且范围足够狭窄，以至于实验能够真正朝着目标取得进展。如果我们试图同时添加多个特征或回答多个问题，我们可能无法区分它们对结果的单独影响。

- 目标举例：

  - 尝试对管道进行潜在改进 (例如，新的正则化项、预处理选择等) 。
...
**总结**：识别实验目标的科学超参数、干扰超参数和固定超参数。 创建一系列研究，用于比较科学超参数的不同值，同时优化干扰超参数。 选择干扰超参数的搜索空间，以平衡资源成本和科学价值。
...
- 对于给定的目标，所有超参数将被归为以下三类之一：**科学超参数**、**干扰超参数**或**固定超参数**。

  - **科学超参数**是我们试图衡量其对模型性能影响的超参数。

  - **干扰超参数**是需要优化的超参数，以便公平比较科学超参数的不同值。这类似于统计学中的[干扰参数](https://en.wikipedia.org/wiki/Nuisance_parameter)概念。

  - **固定超参数**的值在当前轮实验中是固定的。这些超参数的值在比较科学超参数的不同值时不需要 (或我们不希望) 改变。
    - 固定某些超参数进行一系列实验时，我们必须承认实验得出的结论可能对固定超参数的其他设置无效。换句话说，固定超参数会为我们从实验中得出的任何结论带来限制。

- 例如，如果我们的目标是“确定具有更多隐藏层的模型是否会降低验证误差”，那么隐藏层的数量就是一个科学超参数。

  - 学习率是一个干扰超参数，因为只有在为每个不同层数单独调整学习率的情况下，我们才能公平比较具有不同隐藏层数的模型 (最佳学习率通常取决于模型架构) 。

  - 激活函数可以是一个固定超参数，如果我们在先前实验中确定激活函数的最佳选择对模型深度不敏感，或者如果我们愿意将隐藏层数量的结论限制为仅涵盖这种特定的激活函数选择。另一种选择是，如果我们准备为每个隐藏层数量单独调整它，它可以成为一个干扰参数。

- 特定超参数是科学超参数、干扰超参数还是固定超参数并不是该超参数本身的固有属性，而是会根据实验目标而改变。
  - 例如，激活函数的选择可以是科学超参数 (ReLU 还是 tanh 对我们的问题更好？) 、干扰超参数 (当我们允许几种不同的激活函数时，最好的 5 层模型是否比最好的 6 层模型更好？) 或固定超参数 (对于 ReLU 网络，在特定位置添加批量归一化是否有帮助？) 。

- 在设计新一轮实验时，我们首先要确定实验目标的科学超参数。
  - 在这一阶段，我们将所有其他超参数都视为干扰超参数。

- 接下来，我们将一些干扰超参数转换为固定超参数。

  - 如果资源无限，我们将把所有非科学超参数都留为干扰超参数，这样我们从实验中得出的结论就不会受到固定超参数值的限制。

  - 但是，我们尝试调整的干扰超参数越多，我们就越有可能无法为科学超参数的每个设置进行充分的调整，最终导致从实验中得出错误的结论。
    - 如下文[在信息丰富和可负担的实验之间取得平衡](#在信息丰富和可负担的实验之间取得平衡)所述，如需降低这种风险，我们可以增加计算预算，但通常情况下，我们最大的资源预算不足以调整所有非科学超参数。
  - 当我们判断固定某个干扰超参数带来的限制性小于将其作为干扰超参数包含进去的成本时，我们就会选择将该干扰超参数转换为固定超参数。
    - 给定干扰超参数与科学超参数交互的越多，固定其值就越不利。例如，权重衰减强度的最佳值通常取决于模型大小，因此比较不同模型大小时假设权重衰减的单个特定值不会非常有启发性。


- 虽然我们分配给每个超参数的类型取决于实验目标，但对于某些类别的超参数，我们有以下经验法则：

  - 在各种优化器超参数 (例如学习率、动量、学习率调度参数、Adam beta 等) 中，至少会有一些是干扰超参数，因为它们往往与其他变化互动最多。

    - 它们很少是科学超参数，因为诸如“当前管道最佳学习率是什么？”之类的目标不会提供太多见解——最佳设置很容易随着下一个管道更改而改变。
    - 虽然由于资源限制或我们有特别强烈的证据表明它们不会与科学参数交互时，我们可能会偶尔固定其中的一些，但我们通常应该假设需要单独调整优化器超参数才能在科学超参数的不同设置之间进行公平比较，因此不应该固定。
      - 此外，我们没有先验理由更喜欢一个优化器超参数值而不是另一个 (例如，它们通常不会以任何方式影响前向传递或梯度的计算成本) 。


  - 相比之下，优化器的选择通常是科学超参数或固定超参数。

    - 如果我们的实验目标涉及公平比较两个或更多不同的优化器，那么它就是一个科学超参数 (例如，确定给定步数内哪个优化器产生最低的验证误差) 。
    - 或者，由于各种原因，我们可能会将其设置为固定超参数，原因包括： (1) 先前实验使我们相信，我们问题的最佳优化器对当前的科学超参数不敏感； (2) 我们更喜欢使用这个优化器比较科学超参数的值，因为它的训练曲线更容易理解； (3) 我们更喜欢使用这个优化器，因为它比其他选项更省内存。
  - 正则化技术引入的超参数通常是干扰超参数，但我们是否包含正则化技术本身则是一个科学超参数或固定超参数。
    - 例如，dropout 会增加代码复杂性，因此在决定是否包含它时，我们将 “no dropout” 和 “dropout” 设置为科学超参数，并将 dropout 率设置为干扰超参数。
      - 如果我们根据此实验决定将 dropout 添加到我们的管道中，那么 dropout 率在未来的实验中将成为干扰超参数。
  - 架构超参数通常是科学超参数或固定超参数，因为架构更改会影响服务和训练成本、延迟和内存需求。
    - 例如，层数通常是科学超参数或固定超参数，因为它往往会对训练速度和内存使用率产生巨大影响。

- 在某些情况下，干扰超参数和固定超参数的集合将取决于科学超参数的值。

  - 例如，假设我们试图确定 Nesterov momentum 和 Adam 中哪个优化器可以产生最低的验证误差。科学超参数是优化器，它取值 `{"Nesterov_momentum", "Adam"}`。值 `optimizer="Nesterov_momentum"` 引入了冗余/固定超参数 `{learning_rate, momentum}`，而值 `optimizer="Adam"` 引入了冗余/固定超参数 `{learning_rate, beta1, beta2, epsilon}`。
  - 仅针对科学超参数的某些值存在的超参数称为**条件超参数**。
  - 我们不应该假设两个条件超参数相同，仅仅因为它们具有相同的名称！在上述示例中，名为 `learning_rate` 的条件超参数对于 `optimizer="Nesterov_momentum"` 和 `optimizer="Adam"` 来说是不同的超参数。它在两种算法中的作用相似 (虽然并不完全相同) ，但在每个优化器中效果良好的值的范围通常相差几个数量级。
...
在设计研究或一系列研究时，我们需要分配有限的预算来充分实现以下三个期望：

- 足够比较科学超参数的不同值。
- 在足够大的搜索空间上调整干扰超参数。
- 足够密集地采样干扰超参数的搜索空间。

我们更好地实现这三个期望，从实验中就能提取更多的见解。

尽可能比较科学超参数的更多值，可以拓宽我们从实验中获得的见解范围。

包含尽可能多的干扰超参数，并允许每个干扰超参数在尽可能宽的范围内变化，会增加我们对每个科学超参数配置的搜索空间中存在干扰超参数“良好”值的信心。

否则，我们可能会因为没有搜索科学参数可能存在更好值的干扰参数空间区域，从而对科学参数的值进行不公平的比较。

尽可能密集地采样干扰超参数的搜索空间，可以增加我们对搜索过程中找到搜索空间中可能存在的任何好的干扰超参数设置的信心。

否则，我们可能会因为一些值在干扰超参数的采样过程中更幸运而导致科学参数的值之间进行不公平的比较。

不幸的是，这三个方面的任何改进都需要增加试验次数 (从而增加资源成本) ，或者找到一种方法节省其他方面的资源。

每个问题都有其自身的特殊性和计算限制，因此如何在这些三个期望之间分配资源需要一定的领域知识。

在进行研究之后，我们总是会尝试了解这项研究是否足够好地调整了干扰超参数 (即足够广泛地搜索了足够大的空间) 以便公平比较科学超参数 (如下文所述) 。
